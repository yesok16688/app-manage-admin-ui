...
import { fetchDeleteRegionBlacklistInfo, fetchGetRegionBlacklist } from '@/service/api';
import { $t } from '@/locales';
import { useAppStore } from '@/store/modules/app';
import { useTable, useTableOperate } from '@/hooks/common/table';
import { getOptions } from '@/store/modules/app/shared';
import OperateDrawer from './modules/blacklist-drawer.vue';
import Search from './modules/blacklist-search.vue';
...
const {
  ...
  columns: () => [
    ...
    {
      key: 'region_code',
      title: $t('page.regionBlacklist.region'),
      align: 'center',
      minWidth: 80,
      render: row => {
    ...
    {
      key: 'is_enable',
      title: $t('page.regionBlacklist.isEnable'),
      align: 'center',
      width: 80,
      render: row => {
...
async function handleDelete(id: number) {
  const { error } = await fetchDeleteRegionBlacklistInfo(id);
  if (!error) {
    onDeleted();
  }
}
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    ...
    <NCard
      :title="$t('page.regionBlacklist.title')"
      :bordered="false"
      size="small"
